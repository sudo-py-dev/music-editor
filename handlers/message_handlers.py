...
from tools.database import Chats, AudioFiles, Users
...
async def private_message_handler(client: Client, message: Message, language: str):
    ...
    user = Users.get_waiting_for(user_id)
    if not user:
    ...
    max_length = 64
    if (wait_for := user.get("waiting_for")) and audio_id:
        if wait_for == "cut":
            if not message.text:
                await message.reply(messages.wait_for_cut)
                await message.delete()
                return
            cat = message.text.strip()
            try:
                start_time, end_time = parse_cut_range(cat)
            except ValueError:
                await message.reply(messages.wait_for_cut)
                await message.delete()
                return
            audio_file = AudioFiles.update(user_id=user_id,
                                               audio_id=audio_id,
...
@with_language
async def audio_message_handler(client: Client, message: Message, language: str):
    user_id = message.from_user.id
    if message.audio:
    ...
    audio_file = AudioFiles.create(user_id=user_id,
                                   file_id=file_id,
                                   file_name=file_name,
                                   file_size=file_size,
                                   title=file_title,
