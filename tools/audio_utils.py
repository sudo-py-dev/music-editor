...
def parse_cut_range(cut_str: str) -> tuple[float, float]:
    """
    ...
    """
    if not cut_str:
        raise ValueError("Empty cut range string")

    # Split by dash or similar separators
    parts = re.split(r"\s*[-–—]\s*", cut_str.strip())  # supports "-", "–", "—"
    if len(parts) != 2:
        raise ValueError(f"Invalid cut range format: '{cut_str}'")
    ...
    if start_sec < 0 or end_sec < 0:
        raise ValueError("Start or end time cannot be negative.")
